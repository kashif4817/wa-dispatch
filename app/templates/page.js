...
import { Eye, MessageSquareText, Pencil, Plus, RefreshCw, Search, Trash2, X } from "lucide-react";
    ...
    <Shell noPadding>
      <div className="flex h-full flex-col overflow-hidden bg-neutral-100 dark:bg-zinc-950">
        <div className="flex items-center gap-2 border-b border-neutral-200 bg-neutral-50 px-4 py-2.5 dark:border-zinc-800 dark:bg-zinc-900">
          <span className="mr-1 text-[14px] font-semibold text-neutral-800 dark:text-zinc-100">Templates</span>
        ...
        <div className="flex-1 overflow-auto p-5">
          <div className="grid gap-5 lg:grid-cols-[380px_1fr]">
            <Section title="New Template" eyebrow="Add template" icon={MessageSquareText}>
              <div className="flex flex-col gap-3">
                <input
                  className="field h-10 px-3 text-[14px]"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Template name"
                />
                <textarea
                  className="field p-3 text-[14px] leading-7"
                  rows={9}
                  value={message}
